...
First note that the call of `setRefClass` is quite similar to the call of `setClass` when S4 was used. You can find the documentation by calling `?setRefClass`. Note that the fields of the class are defined via the argument `fields` (in `setClass` for S4 they were called `slots`). Many further options are available, as for example declaring inheritance relationships or a mechanism to lock fields to make them private. We will not discuss these details here any further and refer you to the documentation within R. It is important to observe that the call of `setRefClass` returns a generator function which we assign to the variable `DataContainer`. In fact, `setClass` for S4 returns a generator function as well, but for Reference Classes it is more important to be able to access it, as can be seen now. In the definition of `dataContainer` (our constructor or helper function that can be used to generate objects of class `DataContainer`) we call the `new` function of `DataContainer` by using the `$` operator. This nicely illustrates how methods of an object are accessed (i.e., `[object_name]$[method_name]()`). Next we call the `methods` method of `DataContainer` and define the methods `initialize`, `doComputation` and `show` for the class. Further, we define in the same call to `methods` the getter `getResult` and the setter `setData`. Note that we have not used the `accessors` function of the generator as this would not allow us to implement the special behaviour we would like to see. Further, note that in the definition of the functions we use `.self` to refer to the object from within these functions are called. You may be familiar with this from other programming languages (e.g., `this` in Java).

Now we use the Reference Class `DataContainer` above to see that it indeed fascilitates modify-in-place:

```{r}
dC1 <- dataContainer(rnorm(5))
...
dataContainer_S4 <- function(data) {
  dC <- new("DataContainer_S4", data = data)
  return(dC)
}
...
dC1 <- dataContainer_S4(rnorm(5))

dC1         # no result set yet!
result(dC1) # calling getResult() will compute the result
dC1         # but the result is not set
result(dC1) # calling getResult() a second time returns the result,
            # but doesn't do the computation again
```
