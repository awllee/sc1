...
One can use the operator `=` instead of `<-`, but this is not recommended.
...
A particularly problem is when a programmer expects a variable to be defined locally within a function but it is not, and instead a global variable with the same name is used.
...
In R it is often preferable for performance reasons to use loop replacements such as `apply`, `lapply`. We will consider this in more detail later.
