...
Note that `foreach` is not the same as a paraellel for loop. It actually outputs something, typically a list of the same length as the number of iterates in the for loop. `%do%` evaluates the expression sequentially, while `%dopar%` evaluates it in parallel.
