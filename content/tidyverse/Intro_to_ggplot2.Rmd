...
Here we introduce the main features of the [`ggplot2`](https://cran.r-project.org/web/packages/ggplot2/) `R` package, but we refer to ["The Layered Grammar of Graphics"](http://vita.had.co.nz/papers/layered-grammar.pdf) and to there [relevant chapter](https://r4ds.had.co.nz/data-visualisation.html) of "R for Data Science" for more details.  
...
```
Base R plots are generally called for their side effects, rather that for their return value. For example, the `R` code executed within `plot.default` renders the scatterplot shown above, but this function does not return anything useful: 
```{r, fig.show = 'hide'}
...
pl
```
Remember that evaluating an object, `pl` in this case, on the console triggers a call to the generic `print` function. Given that `pl` has class `ggplot`, this dispatches to the `print.ggplot` method (not exported by `ggplot2`, but you can see the code by doing `ggplot2:::print.ggplot`). Of course, we can do the whole thing in one step and get the same result (not shown):
```{r, fig.show = 'hide'}
ggplot(data = mcycle) + geom_point(mapping = aes(x = times, y = accel))
```

The code above shows that the first difference between `ggplot2` and `graphics` plotting methods is that `ggplot2` explicitly separates the plot-building phase (initial plot creation using `ggplot`, followed by addition of layers such as `geom_point`) from the rendering phase (performed by `print`). `ggplot2` also distinguishes the data set that contains the variables to be used in the plot (`mcycle` in this example, which is passed to the initial `ggplot` function) from the variables names that are specified when calling the specific layers (`accel` and `times`, which are passed to `geom_point`). A generic `ggplot2` template might look something like this:    
```{r, eval = FALSE}
